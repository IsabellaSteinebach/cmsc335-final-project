# Random Dog Generator
### DaaS (Dogs-as-a-Service): the internet's biggest collection of open source dog pictures

Submitted by: ...Rose Smith (superterp)\
Group Members: Isabella Steinebach (isteineb), Kim Luu (...), Dhanya Desai (...)\
App Description: Allow users to retrieve a picture of a dog and stores/retrieves all of their past dog pictures.\
YouTube Video Link: ...\
APIs: Dog API (https://dog.ceo/dog-api/) - The internet's biggest collection of open source dog pictures\
Contact Email: isteineb@terpmail.umd.edu\
Deployed App Link: ...


